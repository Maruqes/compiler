...
void xor64_r_r(uint8_t reg1, uint8_t reg2)
{
    ...
    set_rex_prefix(opcode_bytes, 1,
                   ...
                   0,
                   (reg1 >= REG_R8) ? 1 : 0);
    opcode_bytes[1] = 0x31; // XOR r/m64, r64
    set_modrm(&opcode_bytes[2], MOD_REG_DIRECT, reg2, reg1);
...
void xor64_r_i(uint8_t reg, uint32_t imm32)
{
    ...
    set_rex_prefix(opcode_bytes, 1,
                   ...
                   0,
                   (reg >= REG_R8) ? 1 : 0);
    opcode_bytes[1] = 0x81;                              // XOR r64, imm32
    set_modrm(&opcode_bytes[2], MOD_REG_DIRECT, 6, reg); // /6 for XOR
    memcpy(&opcode_bytes[3], &imm32, sizeof(uint32_t));
...
void xor64_r_m(uint8_t reg1, uint8_t reg2)
{
    ...
    set_rex_prefix(opcode_bytes, 1,
                   ...
                   0,
                   (reg2 >= REG_R8) ? 1 : 0);
    opcode_bytes[1] = 0x33; // XOR r64, r/m64
    set_modrm(&opcode_bytes[2], MOD_1BYTE_DISP, reg1, reg2);
    set_sib(&opcode_bytes[3], SCALE_1, RM_SIB, reg2);
    opcode_bytes[3 + usa_sib] = 0x00;
...
void xor64_r_mi(uint8_t reg1, uint8_t reg2, uint32_t offset)
{
    ...
    set_rex_prefix(opcode_bytes, 1,
                   ...
                   0,
                   (reg2 >= REG_R8) ? 1 : 0);
    opcode_bytes[1] = 0x33; // XOR r64, r/m64 with disp32
    set_modrm(&opcode_bytes[2], MOD_4BYTE_DISP, reg1, reg2);
    set_sib(&opcode_bytes[3], SCALE_1, RM_SIB, reg2);
    memcpy(&opcode_bytes[3 + usa_sib], &offset, sizeof(offset));
